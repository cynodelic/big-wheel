...
// The best parameters are used as default
template <
...
class big_wheel_engine
{
	public:
		...
		// Constructor with 0 to 16 seeds or a seed sequence generator
		big_wheel_engine(
		...
		// Constructor with a seed sequence generator
		template <class SeedSeq>
		big_wheel_engine(SeedSeq& seq)
		{
			seed_impl(
		...
		result_type operator()()
		{
			...
			const std::size_t p1 = (counter+  0)%4096;
			const std::size_t p2 = (counter+  8)%4096;
			const std::size_t p3 = (counter+ 18)%4096;
			const std::size_t p4 = (counter+ 30)%4096;
			const std::size_t p5 = (counter+ 44)%4096;
			const std::size_t p6 = (counter+ 60)%4096;
			const std::size_t p7 = (counter+ 78)%4096;
			const std::size_t p8 = (counter+ 98)%4096;
			const std::size_t p9 = (counter+120)%4096;
			
			// Start mixing
			const result_type tmp_11 = (state[p1] << rs1) | (state[p2] >> (32-rs1));
			const result_type tmp_12 = (state[p3] << rs2) | (state[p4] >> (32-rs2));
			const result_type tmp_13 = (state[p5] << rs3) | (state[p6] >> (32-rs3));
			const result_type tmp_14 = (state[p7] << rs4) | (state[p8] >> (32-rs4));
			
			const result_type tmp_21 = ((tmp_11 << rm1) | (tmp_12 >> (32-rm1)));
			const result_type tmp_22 = ((tmp_13 << rm2) | (tmp_14 >> (32-rm2)));
			
			const result_type tmp_3 = (tmp_21 & m) | (tmp_22 & ~m);
			
			const result_type tmp_4 = rot_r(state[p9] ^ tmp_3,rf);
		...
		friend std::basic_istream<CharT,Traits>& operator>>(
			...
			big_wheel_engine<m,rs1,rs2,rs3,rs4,rm1,rm2,rf>& bw)
		{
	...
	private:
		...
		// Counter (for picking elements from state)
		std::size_t counter;
		...
		inline result_type smix(result_type x,std::uint32_t *sb)
		{
			return result_type(
				(sb[(x >> 24)&0xff] << 24)|
				(sb[(x >> 16)&0xff] << 16)|
		...
		// Seed with a seed sequence generator
		template <class SeedSeq>
		void seed_impl(std::true_type,SeedSeq& seq)
		{
			// Generate a set of seeds
			result_type sset[4096] = {0x0};
			seq.generate(&sset[0],&sset[state_size]);
			...
			{
				rn_1[i] = (sset[(i+ 0)%4096] << 13) | (sset[(i+ 4)%4096] >> 19);
		...
		// Seed with 0 to 16 values
		void seed_impl(std::false_type,
			const result_type& s01 = 0x0,const result_type& s02 = 0x0,const result_type& s03 = 0x0,const result_type& s04 = 0x0,
			const result_type& s05 = 0x0,const result_type& s06 = 0x0,const result_type& s07 = 0x0,const result_type& s08 = 0x0,
			const result_type& s09 = 0x0,const result_type& s10 = 0x0,const result_type& s11 = 0x0,const result_type& s12 = 0x0,
			const result_type& s13 = 0x0,const result_type& s14 = 0x0,const result_type& s15 = 0x0,const result_type& s16 = 0x0)
...
// Criteria:
// - There are no elements such as sbox_*[N] = N for all N.
// - There is an only element such as sbox_*[N] xor N = N.
// - There should be, at least, 128 elements, such as their
//   Hamming distance between sbox_*[N] and N is above or
//   equals 4.
template <
	std::uint32_t m,
...
// An alias for big_wheel_engine<>
typedef big_wheel_engine<> big_wheel;
